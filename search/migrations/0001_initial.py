# Generated by Django 2.0 on 2019-01-22 09:40

from django.conf import settings
from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
    ]

    operations = [
        migrations.CreateModel(
            name='Collections',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
            ],
            options={
                'db_table': 'collections',
            },
        ),
        migrations.CreateModel(
            name='Comment',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('comment_date', models.DateField()),
                ('comment_text', models.TextField()),
                ('praise', models.IntegerField()),
            ],
            options={
                'db_table': 'comment',
            },
        ),
        migrations.CreateModel(
            name='Label',
            fields=[
                ('id', models.IntegerField(default=0, primary_key=True, serialize=False)),
                ('label_name', models.CharField(max_length=10)),
            ],
            options={
                'db_table': 'label',
            },
        ),
        migrations.CreateModel(
            name='Singer',
            fields=[
                ('id', models.IntegerField(default=4292, primary_key=True, serialize=False)),
                ('singer_name', models.CharField(max_length=50)),
                ('singer_img', models.URLField()),
                ('album_img', models.URLField()),
                ('singer_info', models.TextField()),
                ('singer_fans', models.CharField(max_length=20)),
                ('singer_albums', models.CharField(max_length=200)),
            ],
            options={
                'db_table': 'singer',
            },
        ),
        migrations.CreateModel(
            name='Songs',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('song_id', models.CharField(max_length=20)),
                ('song_name', models.CharField(max_length=50)),
                ('song_img', models.URLField()),
                ('song_lyrics', models.TextField()),
                ('song_album', models.CharField(max_length=50)),
                ('song_time', models.TimeField()),
                ('song_url', models.CharField(max_length=150)),
                ('play_num', models.IntegerField()),
                ('download_num', models.IntegerField()),
                ('is_free', models.BooleanField()),
                ('label', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='search.Label')),
                ('singer', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='search.Singer')),
            ],
            options={
                'db_table': 'songs',
            },
        ),
        migrations.CreateModel(
            name='Wheel',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('img_name', models.CharField(max_length=100)),
                ('img_url', models.URLField()),
            ],
            options={
                'db_table': 'wheel',
            },
        ),
        migrations.AddField(
            model_name='comment',
            name='songs',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='search.Songs'),
        ),
        migrations.AddField(
            model_name='comment',
            name='user',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL),
        ),
        migrations.AddField(
            model_name='collections',
            name='songs',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='search.Songs'),
        ),
        migrations.AddField(
            model_name='collections',
            name='user',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL),
        ),
    ]
